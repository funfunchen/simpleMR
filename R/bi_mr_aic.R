...
bi_mr_aic <- function(X.set, Y.set){
  ...
  beta.xx <- dplyr::select(X.set, X_x_beta) %>% unlist() %>% unname();
  beta.xy <- dplyr::select(X.set, X_y_beta) %>% unlist() %>% unname();
  sebeta.xx <- dplyr::select(X.set, X_x_se) %>% unlist() %>% unname();
  sebeta.xy <- dplyr::select(X.set, X_y_se) %>% unlist() %>% unname();
  
  # Y.set ascertainment data
  beta.yx <- dplyr::select(Y.set, Y_x_beta) %>% unlist() %>% unname();
  beta.yy <- dplyr::select(Y.set, Y_y_beta) %>% unlist() %>% unname();
  sebeta.yx <- dplyr::select(Y.set, Y_x_se) %>% unlist() %>% unname();
  sebeta.yy <- dplyr::select(Y.set, Y_y_se) %>% unlist() %>% unname();
